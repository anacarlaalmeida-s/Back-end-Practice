...
const { validar_cpf_email, validarConta, validarSenha } = require('./validacoes');

function listarTodasAsContas(req, res) {

    if (req.query.senha_banco !== "Cubos123Bank") {
        return res.status(401).json({ erro: "Senha de acesso incorreta ou não informada!" });
    };
...
function criarConta(req, res) {
    ...
    if (!req.body.data_nascimento) {
        return res.status(400).json({ mensagem: "O campo 'Data Nascimento' é obrigatório." })
    };
...
function atualizarUsuarioConta(req, res) {
    ...
    if (!req.body.nome
        ...
        && !req.body.email
        && !req.body.senha) {
        return res.status(400).json({ mensagem: "Para prosseguir, insira pelo menos um dado a ser alterado." })
    }
...
function saldo(req, res) {
    ...
    if (!req.query.numero_conta || !req.query.senha) {
        return res.status(400).json({ mensagem: "Para prosseguir informe o número e a senha da conta." })
    };
...
function excluirConta(req, res) {
    ...
    const saldoConta = bancoDeDados.contas.find(
        (conta) => conta.saldo >= 1
    );

    if (saldoConta) {
        return res.status(400).json({ mensagem: "É necessário zerar o saldo da conta antes de excluí-la." })
    };
...
module.exports = { listarTodasAsContas, criarConta, atualizarUsuarioConta, saldo, extrato, excluirConta };