...
//não deu tempo de refatorar as validações aqui :(
...
function depositar(req, res) {
    ...
    if (!req.body.valor || req.body.valor <= 0) {
        return res.status(400).json({ mensagem: "Informe o valor que será depositado." })
    };
    ...
    if (!contas) {
        return res.status(404).json({ mensagem: "Não existe conta cadastrada no número informado." })
    };
...
function sacar(req, res) {
    ...
    if (!req.body.numero_conta) {
        return res.status(400).json({ mensagem: "É necessário informar o número da sua conta." })
    };
...
function transferir(req, res) {
    ...
    if (!req.body.valor || req.body.valor < 1) {
        return res.status(400).json({ mensagem: "Informe o valor que será transferido." })
    };
    ...
    if (!contaOrigem) {
        return res.status(404).json({ mensagem: "Verifique sua conta, não existe conta cadastrada no número informado." })
    };

    if (!contaDestino) {
        return res.status(404).json({ mensagem: "Verifique a conta de destino, não existe conta cadastrada no número informado." })
    };
